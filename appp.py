...
import io, re, collections, html as htmllib
...
def translate_all(word: str, ctx_list: list, src: str, tgt: str, engine: str) -> dict:
    """แปลคำ + ประโยคบริบท คืน dict ที่ safe สำหรับ HTML"""
    main = translate_text(word, src, tgt, engine)
    ctx_trans = []
    for sent_html in ctx_list[:4]:
        # ลบ HTML tag ออกก่อนส่งแปล
        orig  = re.sub(r'<[^>]+>', '', sent_html).strip()
        trans = translate_text(orig, src, tgt, engine)
...
def tokenize_th(text: str, min_len: int) -> list:
    return [t for t in re.findall(r'[\u0e00-\u0e7f]+', text) if len(t) >= min_len]
...
def split_sentences(text: str, lang: str) -> list:
    sents = re.split(r'(?<=[.!?])\s+', text) if lang == 'en' \
            else re.split(r'(?<=[.!?ฯ])\s*\n*|\n{2,}', text)
    return [s.strip() for s in sents if len(s.strip()) > 10]


def get_contexts(word: str, sentences: list, lang: str, max_ctx: int = 6) -> list:
    """คืน list ของ HTML strings (ข้อความ escaped, คำ highlight ด้วย <mark>)"""
    pattern = re.compile(
    ...
    for sent in sentences:
        if pattern.search(sent):
            # escape HTML พิเศษก่อน แล้วค่อย wrap mark
            safe    = htmllib.escape(sent)
            pattern2 = re.compile(
                r'\b' + re.escape(htmllib.escape(word)) + r'\b' if lang == 'en'
                else re.escape(htmllib.escape(word)),
                re.IGNORECASE,
            )
            highlighted = pattern2.sub(lambda m: f'<mark>{m.group(0)}</mark>', safe)
...
def build_chart(df_top: pd.DataFrame, palette_name: str) -> plt.Figure:
    ...
    for bar, val in zip(bars, counts_rev):
        ax.text(bar.get_width() + max_val * 0.012,
                ...
                color="#4a5888", fontsize=8.5, fontfamily="monospace")
    ax.set_xlim(0, max_val * 1.18)
    ax.set_xlabel("จำนวนครั้ง", color="#8a96b8", fontsize=9)
...
def render_context_panel(word, ctx_list, freq, pct) -> str:
    ...
    more = (f'<div class="ctx-more">… และอีก {freq - len(ctx_list)} ครั้ง</div>'
...
def render_trans_panel(result, src_lang, engine_key) -> str:
    """
    render ผลแปล — escape ข้อความทุกตัวก่อนใส่ใน HTML
    แก้ปัญหา HTML tag รั่วออกมาเป็นข้อความ
    """
    engine_names = {"google": "Google Translate", "mymemory": "MyMemory"}
    tgt_label    = "ไทย" if src_lang == 'en' else "English"
    engine_name  = htmllib.escape(engine_names.get(engine_key, engine_key))

    # escape คำแปลหลัก
    main_trans = htmllib.escape(result.get("main", "—"))

    ctx_items = ""
    for c in result.get("context_translations", []):
        orig  = htmllib.escape(c.get("orig",  "")[:150])
        trans = htmllib.escape(c.get("trans", "") )
        if not orig and not trans:
            continue
        ellipsis = "…" if len(c.get("orig","")) > 150 else ""
        ctx_items += f"""
        <div class="trans-ctx-item">
          <div class="trans-ctx-orig">📄 {orig}{ellipsis}</div>
          <div class="trans-ctx-translated">🔁 {trans}</div>
        </div>"""

    note = htmllib.escape(result.get("note", ""))
    no_ctx = '<div class="trans-ctx-item" style="color:#8aaa98;">ไม่มีบริบท</div>'

    return f"""<div class="trans-panel">
      <div class="trans-engine-tag">
        🔤 แปล → {tgt_label}
        <span class="engine-badge">✓ {engine_name}</span>
        <span class="engine-badge">FREE</span>
      </div>
      <div class="trans-main">{main_trans}</div>
      <hr class="trans-divider">
      <div class="trans-ctx-label">บริบทที่แปลแล้ว</div>
      {ctx_items if ctx_items else no_ctx}
...
if uploaded:
        ...
        with c_trans:
            ...
            if need_new:
                with st.spinner(f'🔤 กำลังแปล "{selected_word}" ...'):
                    try:
                        result = translate_all(selected_word, ctx_list, lang, tgt_lang, engine_key)
                        st.session_state.trans_result  = result
                        st.session_state.trans_word    = selected_word
                        st.session_state.trans_engine  = engine_key
                    except Exception as e:
                        st.markdown(
                            f'<div class="trans-panel">'
                            f'<div class="trans-error">❌ แปลไม่สำเร็จ: {htmllib.escape(str(e))}<br>'
                            f'<span style="font-size:.75rem;">ลองเปลี่ยน engine หรือตรวจสอบ internet</span>'
            ...
            if result:
                st.markdown(render_trans_panel(result, lang, engine_key), unsafe_allow_html=True)
    ...
    <div class="info-box">
      ...
      รองรับภาษาอังกฤษและไทย · แปลฟรีด้วย Google Translate / MyMemory
      </span>
    </div>""", unsafe_allow_html=True)
